...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:cbc="urn:oasis:names:specification:ubl:schema:xsd:CommonBasicComponents-2" xmlns:cac="urn:oasis:names:specification:ubl:schema:xsd:CommonAggregateComponents-2" xmlns:ubl="urn:oasis:names:specification:ubl:schema:xsd:CommonAggregateComponents-2" xmlns:fitek="java:oc.tools.Validators" exclude-result-prefixes="fitek">
	...
	<xsl:template match="RET_DOK">
		...
		<xsl:variable name="SenderRegNumber" ><xsl:text>Sender_Reg_Number</xsl:text></xsl:variable>
		<xsl:variable name="SenderVATNumber" ><xsl:text>Sender_VAT_Number</xsl:text></xsl:variable>
		<xsl:variable name="SenderName" ><xsl:text>Sender_Name</xsl:text></xsl:variable>
		<xsl:variable name="SenderAddress" ><xsl:text>Sender_Address</xsl:text></xsl:variable>
		<xsl:variable name="SenderCountryCode" ><xsl:text>LT</xsl:text></xsl:variable>
		<xsl:variable name="ReceiverCountryCode" ><xsl:text>LT</xsl:text></xsl:variable>
	...
	<!-- <xsl:variable name="senderRegNumber" ><xsl:text>senderRegNumber</xsl:text></xsl:variable> -->
	<xsl:variable name="senderVatNumber" ><xsl:text>senderVatNumber</xsl:text></xsl:variable>
	<xsl:variable name="senderName" ><xsl:text>senderName</xsl:text></xsl:variable>
	<xsl:variable name="senderAddress" ><xsl:text>senderAddress</xsl:text></xsl:variable>
	<xsl:variable name="senderCountryCode" ><xsl:text>LT</xsl:text></xsl:variable>
	
	<xsl:variable name="receiverCountryCode"><xsl:text>LT</xsl:text></xsl:variable>
	<xsl:variable name="Total_I07_SUMA"><xsl:value-of select="sum(I07/I07_SUMA)"/></xsl:variable>
		...
		<xsl:variable name="yIDate">                                              <!--____9.___account date____-->
		...
		<!-- kol kas nezinomi kintamieji iš rivilės formato -->
		...
		<cac:Party>
		  ...
		  <cac:PartyName>
			<cbc:Name><xsl:value-of select="normalize-space(I06_PAV)" /></cbc:Name>
		  </cac:PartyName>
		  <cac:PostalAddress>
			<cbc:StreetName><xsl:value-of select="normalize-space(I06_ADR)" /></cbc:StreetName>
			<cac:Country>
...
<!-- kol kas nezinomi kintamieji iš rivilės formato -->
		  ...
		  <cac:PartyLegalEntity>
			<cbc:RegistrationName><xsl:value-of select="normalize-space(I06_PAV)" /></cbc:RegistrationName>
			<cbc:CompanyID><xsl:value-of select="normalize-space(I06_KODAS_KS)" /></cbc:CompanyID>
		  </cac:PartyLegalEntity>
		</cac:Party>
	  </cac:AccountingCustomerParty>
	...
	<!-- <xsl:message terminate="no">Debug: Current Index - <xsl:value-of select="I07/I07_MOKESTIS_P"/></xsl:message> -->

	<!-- // $TaxCategoryID = ( -->
		<!-- IF ${I06_KODAS_XS} in ('PVM1', 'PVM21')                THEN return 'S' -->
		<!-- ELSE IF ${I06_KODAS_XS} in ('PVM5', 'NE-PVM-OBJEKTAS') THEN return 'E' -->
		<!-- ELSE IF ${I07_MOKESTIS_P} = 0                          THEN return 'Z' -->
		<!-- ELSE return 'S' -->
	   <!-- ) -->
	<!-- // -->

	  <cac:TaxTotal>
		<cbc:TaxAmount currencyID="EUR"><xsl:value-of select="I06_SUMA_PVM" /></cbc:TaxAmount>
		<cac:TaxSubtotal>
		  <cbc:TaxableAmount currencyID="EUR"><xsl:value-of select="format-number(xs:decimal(normalize-space($Total_I07_SUMA)), '0.00')" /></cbc:TaxableAmount>
		  <cbc:TaxAmount currencyID="EUR"><xsl:value-of select="I06_SUMA_PVM" /></cbc:TaxAmount>
		  <cac:TaxCategory>
			<cbc:ID><xsl:value-of select="$taxCode" /></cbc:ID>
			<cbc:Percent><xsl:value-of select="normalize-space(I07[1]/I07_MOKESTIS_P)" /></cbc:Percent>
			<cac:TaxScheme>
			  <cbc:ID>VAT</cbc:ID>
			</cac:TaxScheme>
		  </cac:TaxCategory>
	  ...
	  <cac:LegalMonetaryTotal>
		<cbc:LineExtensionAmount currencyID="EUR"><xsl:value-of select="format-number(xs:decimal(normalize-space($Total_I07_SUMA)), '0.00')" /></cbc:LineExtensionAmount>
		<cbc:TaxExclusiveAmount currencyID="EUR"><xsl:value-of select="format-number(xs:decimal(normalize-space($Total_I07_SUMA)), '0.00')" /></cbc:TaxExclusiveAmount>
		<cbc:TaxInclusiveAmount currencyID="EUR"><xsl:value-of select="I06_SUMA" /></cbc:TaxInclusiveAmount>
		<cbc:PayableAmount currencyID="EUR"><xsl:value-of select="I06_SUMA" /></cbc:PayableAmount>
	  </cac:LegalMonetaryTotal>
	...
	<xsl:for-each select="I07">
        ...
        <cac:InvoiceLine>    
            <cbc:ID><xsl:value-of select="normalize-space(I07_EIL_NR)" /></cbc:ID>
            <cbc:InvoicedQuantity unitCode="H87"><xsl:value-of select="normalize-space(I07_KIEKIS)" /></cbc:InvoicedQuantity>
            <cbc:LineExtensionAmount currencyID="EUR"><xsl:value-of select="normalize-space(I07_SUMA)" /></cbc:LineExtensionAmount>
            <cac:Item>
                <cbc:Description><xsl:value-of select="normalize-space(I07_PAV)" /></cbc:Description>
                <cbc:Name><xsl:value-of select="normalize-space(I07_aprasymas2)" /></cbc:Name>
                <cac:ClassifiedTaxCategory>
                    <!-- <cbc:ID><xsl:value-of select="$TaxCategoryID" /></cbc:ID> -->
                    <cbc:Percent><xsl:value-of select="normalize-space(I07_MOKESTIS_P)" /></cbc:Percent>
                    <cac:TaxScheme>
                        <cbc:ID>VAT</cbc:ID>
                    </cac:TaxScheme>
                </cac:ClassifiedTaxCategory>
            ...
            <cac:Price>
                <cbc:PriceAmount currencyID="EUR"><xsl:value-of select="format-number(xs:decimal(normalize-space(I07_KAINA_BE)), '0.00')" /></cbc:PriceAmount>
            </cac:Price>
        </cac:InvoiceLine>
	</xsl:for-each>
